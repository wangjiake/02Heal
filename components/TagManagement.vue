...
<template>
    <div class="p-4 bg-white rounded-lg shadow">
        <h2 class="text-xl font-bold mb-4">标签管理</h2>

        <form @submit.prevent="saveTag" class="space-y-4 mb-6">
            <div class="mb-4">
...
const getTagsFromLocalStorage = () => {
    if (process.client) {
        const savedTagsData = localStorage.getItem("foodTags");
        if (savedTagsData) {
            return JSON.parse(savedTagsData);
        }
    }
    return [];
};
...
const saveTagsToLocalStorage = (tags) => {
    if (process.client) {
        localStorage.setItem("foodTags", JSON.stringify(tags));
    }
};

// 从 localStorage 获取固定标签的可见性
const getFixedTagsVisibilityFromLocalStorage = () => {
    if (process.client) {
        const fixedTagsVisibility = localStorage.getItem("fixedTagsVisibility");
        if (fixedTagsVisibility) {
            return JSON.parse(fixedTagsVisibility);
        }
    }
    return null;
};

// 保存固定标签的可见性到 localStorage
const saveFixedTagsVisibilityToLocalStorage = () => {
    if (process.client) {
        const visibilityMap = {};
...
// 更新可见标签到 localStorage
const updateVisibleTags = () => {
    if (process.client) {
        // 合并固定标签和用户标签中所有可见的
        const visibleTags = mergedTags.value.filter((tag) => tag.visible);
        localStorage.setItem("visibleFoodTags", JSON.stringify(visibleTags));

        // 单独保存固定标签的可见性
        saveFixedTagsVisibilityToLocalStorage();

        // 触发自定义事件，通知其他组件标签已更新
        window.dispatchEvent(new Event("foodTagsUpdated"));
    }
};
...
const saveTag = () => {
    ...
    saveTagsToLocalStorage(savedTags.value);
    updateVisibleTags();
...
// 组件挂载时，从 localStorage 获取保存的标签和固定标签可见性
onMounted(() => {
    const savedTagsData = getTagsFromLocalStorage();
    if (savedTagsData && savedTagsData.length > 0) {
        savedTags.value = savedTagsData;
    }

    // 恢复固定标签的可见性
    const fixedTagsVisibility = getFixedTagsVisibilityFromLocalStorage();
    if (fixedTagsVisibility) {
        fixedTags.value.forEach((tag) => {
